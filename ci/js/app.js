...
// ===================== FLASHCARDS =====================
const FC_CATS = [...new Set(flashData.map(c => c.cat || 'Outros'))];
...
function buildFcList() {
  const idxs = flashData.map((_, i) => i)
    .filter(i => fcCat === 'todas' || (flashData[i].cat || 'Outros') === fcCat);
  fcList = idxs.filter(i => fcFilter === 'todas' || S.fcBox[i] < 2);
  if (fcFilter === 'dificeis') fcList = idxs.filter(i => S.fcBox[i] === 0);
  fcPos = 0;
}
...
function setFcCat(c) {
  fcCat = c;
  document.querySelectorAll('[data-fccat]').forEach(b => b.classList.toggle('sel', b.dataset.fccat === c));
  buildFcList();
  renderFc();
}

function buildFcCats() {
  const row = $('fc-cat-row');
  ['todas'].concat(FC_CATS).forEach(c => {
    const b = document.createElement('button');
    b.className = 'choice-chip' + (c === fcCat ? ' sel' : '');
    b.dataset.fccat = c;
    b.textContent = c === 'todas' ? 'Todas as categorias' : '📖 ' + c;
    b.addEventListener('click', () => setFcCat(c));
    row.appendChild(b);
  });
}
...
function renderFc() {
  ...
  if (!fcList.length) {
    ...
    $('fc-cat-label').textContent = 'conceito';
  ...
  $('fc-cat-label').textContent = card.cat || 'conceito';
